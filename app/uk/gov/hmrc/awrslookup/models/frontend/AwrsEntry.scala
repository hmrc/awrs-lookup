...
object AwrsEntry {
  ...
  def etmpReader(implicit environment: Environment): Reads[AwrsEntry] = (js: JsValue) =>
    ...
    } yield groupMembers match {
      case Some(grpMembers) =>
        Group(awrsRef = awrsRegistrationNumber, registrationDate = startDate, status = awrsStatus, info = wholesaler, members = grpMembers, registrationEndDate = endDate)
      case _                =>
        Business(awrsRef = awrsRegistrationNumber, registrationDate = startDate, status = awrsStatus, info = wholesaler, registrationEndDate = endDate)
    }
