...
class LookupController @Inject() (
  ...
  def lookupByUrn(awrsRef: String): Action[AnyContent] = Action.async { implicit request =>
    val timer = metrics.startTimer(ApiType.LookupByURN)
    lookupService.lookupByUrn(awrsRef) map { response =>
      timer.stop()
  ...
  def processResponse(
      lookupResponse: HttpResponse,
      apiType:        ApiType
    )(implicit
      fjs:            play.api.libs.json.Reads[SearchResult],
      hc:             HeaderCarrier
    ): Result =
  ...
  private def doAuditing(
      apiType:          ApiType,
      action:           String,
      message:          String,
      eventType:        String,
      incrementCounter: ApiType => Unit
